...
void SampleProtocol_proto_protocol_init()
{
  ...
  PF_sensorName = new_poly_field_desc("sensorName", TYPE_STD::STRING , 16 , FORMAT_ascii );
...
std::string SetdataPacket::Sensorname(std::string  val)
{
...
std::string GetdataPacket::Sensorname(std::string  val)
{
  hasField(PF_sensorName,true);
  mSensorname = val;
}
...
std::string RespdataPacket::Sensorname(std::string  val)
{
  hasField(PF_sensorName,true);
  mSensorname = val;
}
