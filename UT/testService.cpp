...
void tpPacket::_setIcd( uint32_t val)
{
  poly_packet_set_field(&mImp, TP_FIELD_ICD, &val, 1);
}

void tpPacket::_setSensorA(int16_t val)
{
  poly_packet_set_field(&mImp, TP_FIELD_SENSORA, &val, 1);
}

void tpPacket::_setSensorB( int val)
{
  poly_packet_set_field(&mImp, TP_FIELD_SENSORB, &val, 1);
}

void tpPacket::_setSensorName( const char* val)
{
  poly_packet_set_field(&mImp, TP_FIELD_SENSORNAME, val, strlen(val));
}

void tpPacket::_setCmd( uint8_t val)
{
  poly_packet_set_field(&mImp, TP_FIELD_CMD, &val, 1);
}
...
uint32_t tpPacket::_getIcd()
{
  uint32_t val;
  poly_packet_get_field(&mImp, TP_FIELD_ICD, &val );
  return val;
}

int16_t tpPacket::_getSensorA())
{
  int16_t val;
  poly_packet_get_field(&mImp, TP_FIELD_SENSORA, &val );
  return val;
}

int tpPacket::_getSensorB()
{
  int val;
  poly_packet_get_field(&mImp, TP_FIELD_SENSORB, &val );
  return val;
}

string tpPacket::_getSensorName()
{
  return (char*) poly_packet_get_field_ptr(&mImp, TP_FIELD_SENSORNAME);
}

uint8_t tpPacket::_getCmd()
{
  uint8_t val;
  poly_packet_get_field(&mImp, TP_FIELD_CMD, &val );
  return val;
}
